...
export class WorkingSiteNolateComponent implements OnDestroy, OnInit {
  ...
  initalizeReport(): void {
    combineLatest([
      this.timeAttendanceService.getWorkingSiteMonthlyTimeAttendances(
        this.siteId,
        this.year,
        this.month + 1
      ),
      this.timeAttendanceService.getWorkingDaySummary(
        this.siteId,
        this.year,
        this.month + 1
      ),
      ...
      (results) => {
        ...
        if (this.workingSiteMonthly) {
          this.site.siteWorkRates
            ...
            .forEach((x) => {
              if (
                this.workingSiteMonthly.filter(
                  (w) => w.startTime === x.startTime && w.endTime === x.endTime
                ).length > 0
              ) {
                this.workingSitePeriod.push({
                  startTime: x.startTime,
                  endTime: x.endTime,
                  workingSiteMonthly: this.workingSiteMonthly.filter(
                    (w) =>
                      w.startTime === x.startTime && w.endTime === x.endTime
                  ),
                });
              }
            });
          this.workingSitePeriod.forEach((s) => {
            s.workingSiteMonthly.forEach((w) => {
              ...
              for (let index = 0; index < 31; index++) {
                ...
                const validWorkingDay = w.workingDay.filter(
                  (f) => new Date(f.workDate).getDate() === day
                );
                const wkd: WorkingDay = {
                  ...
                  isLate: false,
                  status:
                    day > this.numberDays || validWorkingDay.length <= 0
                      ? 0
                      : 1,
                };
                workingDay.push(wkd);
          ...
          for (let index = 0; index < 31; index++) {
            const day = index + 1;
            const date = new Date(this.year, this.month, day);
            const validWorkingDay = workingDaySum.filter(
              (f) => new Date(f.workDate).getDate() === day
            );
            this.workingDaySummary.push({
              workDate: date,
              workerCount:
                validWorkingDay.length <= 0
                  ? 0
                  : validWorkingDay[0].workerCount,
            });
          }
          this.summaryLateMinute = 0;
          this.summaryManday =
            this.workingDaySummary.reduce(function (prevVal, elem) {
  ...
  convertToDateUrlFormat(date: Date) {
    return `${this.padZeroLeft(date.getDate())}-${this.padZeroLeft(
      date.getMonth() + 1
    )}-${date.getFullYear()}`;
  }
